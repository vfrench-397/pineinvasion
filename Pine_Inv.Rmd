...
We'd like to thank Dr. Alejandro Rojas at the University of Arkansas for providing us with the genomic fungal ITS sequence data, and Dr. Rytas Vilgalys from Duke University for collecting the soil samples. Soil was processed by Corinne Vietorisz and Patrycia Glowiak at Boston University for soil C, N, and P. 
...
**Figure 2. Barplot of guild abundance by forest type.** The x-axis displays the categorical variables of forest type. The y-axis is total abundance of taxonomic class. Functional groups seem to differ across forest types which can impact soil nutrients across forest types.

It is important that abundances of functional groups differ across forest types because a taxa's functional group can indicate its role in soil C and nutrient cycling. For example, saprotrophs are usually implicated in C cycling, while mycorrhizal fungi ("symbiotrophs") are usually implicated in N and P cycling.
...
```{r message=FALSE, warning=FALSE, fig.cap= "**Figure 3. Sample dendogram and trait heat map.** Red indicates high values. The first band indicates that there were no outliers. The following three bands depict soil percent nitrogen, percent carbon, and bulk phosphorus respectively."}
...
plotDendroAndColors(geneTree, dynamicColors, "Dynamic Tree Cut", dendroLabels= FALSE, hang=0.03, addGuide= TRUE, guideHang= 0.05, main= "Dendrogram and module colors")
```

**Figure 4. Dendrogram and module colors.** Each leaf of the tree corresponds to a genus, and branches grouping together densely are genera that frequently occur together. The colors on the Dynamic Tree Cut band correspond to the module groupings of genera. No modules have been merged. 
...
```{r}
plot(METree, main= "Clustering of module eigen abundances", xlab= "", sub= "")
...
**Figure 5. Tree clustering of module eigen abundances based on similarity.** Each branch of the tree represents a module. The red line shows the height at which we merged modules, which was 0.7.
...
**Figure 6. Dendrogram and module colors post-merging.** Each leaf of the tree corresponds to a genus, and branches grouping together densely are genera that frequently occur together. The colors on the Dynamic Tree Cut band correspond to the module groupings of genera without mergin. The colors on the Merged dynamic band correspond to the module groupings after merging at a height of 0.7.
...
**Figure 7. Heatmap of module-trait relationships.** Each row represents a module and each column represents a trait. Red cells indicate that the genera in that module are positively associated with the trait, and blue cells indicate that the genera in that module are negatively associated with the trait. The top number in each cell indicates the correlation between the module and the trait, and the number in parentheses below the correlation value is the p-value showing significance of the correlation. Significance indicates that the genera present in that module are frequently associated with either a high or low value of that trait. 

This heatmap looks good, because we can tell that we did not under-merge our modules. If we had not merged our modules enough, you would see blocks of modules next to each other that have similar profiles (for example, large chunks of blue or red modules together). 
...
verboseScatterplot(abs(geneModuleMembership[moduleGenes, column]),
                   ...
                   ylab = "Genera Sig for bulk P",
                   main = paste("MM vs. GS\n"),
                   cex.main = 1.2, cex.lab = 1.2, cex.axis = 1.2, col = module)
```

**Figure 8. Scatterplot of genera significance for bulk P vs. genera module membership in the yellow module.** Each point represents one genus in the module. Module membership in the yellow module for a given genus is a measure of the correlation of the module eigengene and the genus abundance profile. A tighter positive correlation between module membership and genera significance indicates higher significance of the module. "cor" is the correlation value, and "p" indicates the significance of the correlation. 
...
**Figure 9. Barplot of genera in the yellow module vs. their kMEs for the module.** Increasing kME indicates that the genus fits better into the module.
...
**Figure 11. Heatmap of genera abundances in each sample in the yellow module and barplot of bulk soil P.** Columns of the heatmap represent samples and rows represent each genus in the yellow module. Red indicates higher abundance of a genus and light green represents lower abundance. Increasing values on the y-axis of the barplot indicate more bulk P in a soil sample. "PL" samples are from pine plantations, "UN" samples are from uninvaded eucalypt forests adjacent to invaded forests, and "INV" are from pine-invaded eucalypt forests.
...
### Exploring other significant modules
Now, to explore the other significant module, midnightblue, we do the same steps that we did with the yellow module but with %N as the trait of interest. We first created a scatter plot of genera significance for %N against the module membership in the midnightblue module. We can see in this plot that there is a positive correlation between the module and %N, so the module is indeed significant. 

```{r echo=FALSE, out.width="100%", out.height="100%", fig.cap="**Figure 12. Scatterplot of genera significance for %N vs. genera module membership in the midnightblue module.** Each point represents one genus in the module. Module membership in the midnightblue module for a given genus is a measure of the correlation of the module eigengene and the genus abundance profile. A tighter positive correlation between module membership and genera significance indicates higher significance of the module. cor is the correlation value, and p indicates the significance of the correlation."}
knitr::include_graphics(path.expand('/project/bi594/Pine_invasion/Figures/scatterplot_midnightblue_N.png'))
```

The plot of kME by genera for the midnightblue module shows which genera are in the module and how well they belong. 


```{r echo=FALSE, out.width="100%", out.height="100%", fig.cap="**Figure 13. Barplot of genera in the midnightblue module vs. their kMEs for the module.** Increasing kME indicates that the genus fits better into the module."}
knitr::include_graphics(path.expand('/project/bi594/Pine_invasion/Figures/kME_midnightblue.png'))
```

We can see here that the genera *Serendipita*, *Ophiosphaerella*, *Basidioascus*, *Melanoleuca*, and *Parengyodontium* may be key genera associated with low soil N. 

We made the same heatmaps as with the yellow module, plotting the genera abundances in each sample alongside the eigengene abundance barplot and soil %N barplot. 

```{r echo=FALSE, out.width="100%", out.height="100%", fig.cap="**Figure 14. Heatmap of genera abundances in each sample in the midnightblue module and eigengene abundance barplot.** Columns of the heatmap represent samples and rows represent each genus in the midnightblue module. Red indicates higher abundance of a genus and light green represents lower abundance. Higher bars in the barplot indicate higher abundances of genera in the midnightblue module in a given sample. PL samples are from pine plantations, UN samples are from uninvaded eucalypt forests adjacent to invaded forests, and INV are from pine-invaded eucalypt forests."}
knitr::include_graphics(path.expand('/project/bi594/Pine_invasion/Figures/abund_heatmap_midnightblue.png'))
```

```{r echo=FALSE, out.width="100%", out.height="100%", fig.cap="**Figure 15. Heatmap of genera abundances in each sample in the midnightblue module and barplot of soil %N.** Columns of the heatmap represent samples and rows represent each genus in the midnightblue module. Red indicates higher abundance of a genus and light green represents lower abundance. Increasing values on the y-axis of the barplot indicate more total N in a soil sample. PL samples are from pine plantations, UN samples are from uninvaded eucalypt forests adjacent to invaded forests, and INV are from pine-invaded eucalypt forests."}
knitr::include_graphics(path.expand('/project/bi594/Pine_invasion/Figures/trait_heatmap_midnightblue.png'))
```

In these heatmaps, we can see that the abundance of midnightblue module genera in a sample does not correspond as well with high N as did the yellow module genera with low P, and is primarily driven by one sample. However, we still see some variation in the abundance of genera in the midnight blue module with forest type. The invaded forest at the second site has higher abundances of midnightblue module genera, indicating that the fungal communities change with pine invasion.


We additionally explored the midnight blue module that was significantly positively correlated with %C, but we discounted the correlation because it appears to be driven entirely by one outlier genus. 

```{r echo=FALSE, out.width="100%", out.height="100%", fig.cap="**Figure 16. Scatterplot of genera significance for %C vs. genera module membership in the  midnightblue module.** Each point represents one genus in the module. Module membership in the midnightblue module for a given genus is a measure of the correlation of the module eigengene and the genus abundance profile. A tighter positive correlation between module membership and genera significance indicates higher significance of the module. cor is the correlation value, and p indicates the significance of the correlation."}
knitr::include_graphics(path.expand('/project/bi594/Pine_invasion/Figures/scatterplot_midnightblue_C.png'))
```

# Conclusion

In conclusion, we found that total P was negatively correlated with one module of genera and total N was positively correlated with another module of genera, although the relationship with total N is primarily driven by 1 sample. With this analysis, we identified key fungal genera that are associated with low P and high N. Furthermore, abundance of the genera associated with low P varied with forest type, indicating that changes in fungal communities with pine invasion are related to changes in soil nutrients with pine invasion. 

To hopefully draw more conclusive results, our next steps are to 1) increase sample size; and 2) use the original species-level OTU table as input for the WGCNA instead of collapsing to genera. We also hope to assign finer-level functions to taxa from existing functional databases for fungi to see if the taxa in modules associated with high or low soil nutrients have been previously associated with nutrient cycling functions. 

#Citations 

UNITE reference Database: 

Abarenkov, Kessy; Zirk, Allan; Piirmann, Timo; Pöhönen, Raivo; Ivanov, Filipp; Nilsson, R. Henrik; Kõljalg, Urmas (2020): UNITE general FASTA release for Fungi. UNITE Community. 10.15156/BIO/786368
...
bibentry(bibtype="article",
            ...
            author=c("Nhu H Nguyen", "Zewei Song", "Scott T Bates",
                     "Sara Branco", "Leho Tedersoo", "Jon Menke",
                     "Jonathan S Schilling", "Peter G Kennedy"),
            journal="Fungal Ecology",
            volume="20",
            pages="241--248",
            year="2016",
            publisher="Elsevier",
         textVersion = "Nguyen NH, Song Z, Bates ST, Branco S, Tedersoo L, Menke J, Schilling JS, Kennedy PG. 2016. FUNGuild: An open annotation tool for parsing fungal community datasets by ecological guild. Fungal Ecology 20:241–248."
         )
```
